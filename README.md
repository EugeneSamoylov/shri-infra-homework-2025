...
Репозиторий должен быть публичным и настроен таким образом, чтобы никто не мог пушить ветку `main`, но любой человек мог:
...
Скрипты в `package.json` для работы с Docker-образами.
...
1. Флоу с проверками, который запускается при создании / изменении PR:
    ...
    - есть ограничение на мердж изменений, если проверки не прошли
...
2. Флоу создания релиза, который запускается вручную (`on: [workflow_dispatch]`):
    ...
    - загружает docker-образ в Container Registry (реджистри отображаются оба тега)
...
- Server IP: 89.169.179.144
- Application URL: http://89.169.179.144/hw/store